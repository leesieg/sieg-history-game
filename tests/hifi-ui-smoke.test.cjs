...
assert.match(main, /setAttribute\("aria-label", `查看\$\{country\.displayName \|\| country\.name\}`\)/, "玩家国家标识必须随国家切换");
...
assert.match(dialogs, /HIFI_OBJECTIVES_ENGINE\??\.missionStages/, "局势统一界面必须展示战役阶段使命（已从御前会议迁入局势浮窗）");
assert.match(dialogs, /struggleForPolity/, "御前会议必须能感知当前局势（指向局势浮窗）");
...
// --- Task C6: 军团面板按钮图标+短文字 ---
{
  ...
  assert.match(dialogs, /data-army-embark/, "军团面板应提供登船入口");
  assert.match(dialogs, /data-army-disembark/, "军团面板应提供登陆入口");
  assert.match(dialogs, /embarkArmy/, "军团面板应绑定运兵登船");
  assert.match(dialogs, /disembarkArmy/, "军团面板应绑定两栖登陆");
...
// --- 抽屉数据可视化接线（widgets 全量接入五系统） ---
{
  ...
  // 国家：雷达(统治者) / 制度模块 / 双向条(满意) / 量表(合法性、议会、不满)
  assert.match(src, /wd\(\)\.radar\(\[\s*\{ label: "行政"/, "统治者能力用雷达图");
  assert.match(src, /institution-row/, "政制页应展示制度模块行");
  assert.doesNotMatch(src, /data-reform/, "国家政制页不应再提供旧改革槽按钮");
  assert.match(src, /data-faith-authority/, "信仰页应提供宗教权威行动入口");
  assert.match(main, /data-faith-authority/, "main 应绑定宗教权威行动");
  ...
  assert.match(src, /bribe:/, "帝国页应提供贿选入口");
  assert.match(src, /endorse:/, "帝国页应提供教廷认可入口");
  assert.match(src, /征帝国军/, "帝国页应提供征帝国军入口");
  assert.match(main, /bribeImperialElector/, "main 应绑定贿选选帝侯");
  assert.match(main, /papalEndorseImperialCandidate/, "main 应绑定教廷认可候选人");
  assert.match(main, /raiseImperialArmy/, "main 应绑定征帝国军");
...
// --- Task 3.3: 季度总结 ---
{
  assert.match(dialogs, /renderSeasonSummary/, "dialogs 必须提供本季总结渲染");
  assert.match(dialogs, /quarterLedger/, "本季总结必须读 quarterLedger");
  assert.match(dialogs, /收支变化/, "本季总结必须展示收支变化");
  assert.match(dialogs, /战争变化/, "本季总结必须展示战争变化（含阶段翻转）");
  assert.match(dialogs, /外交变化/, "本季总结必须展示外交变化");
  assert.match(dialogs, /人口变化/, "本季总结必须展示人口变化");
  assert.match(dialogs, /战争 −\$\{e\.war\}/, "账本行必须把战争消耗单列（区分维护/事件/战争）");
  assert.match(main, /captureSeasonSnapshot/, "推进季度前必须快照人口/战争用于比对");
  assert.match(main, /renderSeasonSummary/, "推进季度后 main 必须打开本季总结");
  console.log("Task 3.3 季度总结 OK");
}

// --- 统一局势入口：右下角浮窗 + 统一局势窗接线（反馈 #1/#2）---
{
  // 浮窗：main 渲染卡，点开统一局势窗
  assert.match(main, /renderStruggleDock/, "main 必须渲染右下角局势浮窗");
  assert.match(main, /data-struggle-open/, "浮窗卡必须可点开统一局势窗");
  assert.match(main, /openStrugglePanel/, "main 必须有打开统一局势窗的入口");
  // 统一局势窗：dialogs 消费 struggleSummary，渲染阵营/诱因/结局/决议
  assert.match(dialogs, /renderStrugglePanel/, "统一局势窗必须由 dialogs.renderStrugglePanel 渲染");
  assert.match(dialogs, /struggleSummary/, "局势窗必须消费 struggleSummary");
  assert.match(dialogs, /data-struggle-action/, "局势窗必须提供决议按钮（不可用置灰）");
  assert.match(dialogs, /data-struggle-lean="-1"/, "选边决议必须能支持左侧阵营");
  assert.match(dialogs, /data-struggle-lean="1"/, "选边决议必须能支持右侧阵营");
  assert.match(dialogs, /struggle-camp/, "局势窗必须左右展示两大阵营");
  assert.match(dialogs, /struggle-decision/, "局势窗决议按钮需可置灰");
  // 决议执行仍走阶段 gate + 现有引擎
  assert.match(main, /phaseActionGate/, "局势决议必须经 phaseActionGate 校验阶段");
  assert.match(main, /executeStruggleAction/, "局势决议必须委托现有引擎执行");
  assert.match(main, /dataset\.struggleLean/, "选边决议必须把阵营方向传给执行层");
  // 作战室已从军事战争页移除
  assert.doesNotMatch(drawers, /war-room/, "作战室应已从军事抽屉移除（统一到局势浮窗）");
  console.log("统一局势入口接线 OK");
}

// --- Task 6.1: 终局结算弹窗 ---
{
  assert.match(dialogs, /renderStruggleEnding/, "dialogs 必须提供终局结算弹窗渲染");
  assert.match(dialogs, /pendingStruggleEnding/, "终局弹窗必须读 world.pendingStruggleEnding");
  assert.match(main, /renderStruggleEnding/, "推进季度后 main 必须先尝试展示终局结算");
  console.log("Task 6.1 终局结算弹窗 OK");
}
