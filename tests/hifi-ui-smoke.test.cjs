...
// --- Task 4.2: 局势作战室接线 ---
{
  assert.match(drawers, /struggleSummary/, "战争页必须调用 struggleSummary 渲染作战室");
  assert.match(drawers, /data-struggle-action/, "作战室必须提供阶段限定操作按钮");
  assert.match(main, /phaseActionGate/, "main 必须经 phaseActionGate 校验阶段限定操作");
  assert.match(main, /executeStruggleAction/, "main 必须委托现有引擎执行局势操作");
  console.log("Task 4.2 作战室接线 OK");
}
