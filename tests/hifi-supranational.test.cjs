...
world.countries["勃艮第公国"].money = 100;
world.countries["勃艮第公国"].actionPoints.diplomatic = 2;
const burgundyScoreBeforeBribe = w.HIFI_SUPRANATIONAL_ENGINE.electionScores(world, "hre")
  .find(item => item.polity === "勃艮第公国").score;
const bribe = w.HIFI_SUPRANATIONAL_ENGINE.bribeImperialElector(world, "勃艮第公国", "波西米亚王国", 30);
assert.equal(bribe.paid, 30, "贿选应记录实际支付金额");
assert.equal(world.countries["勃艮第公国"].money, 70, "贿选应消耗金钱");
assert.equal(world.countries["勃艮第公国"].actionPoints.diplomatic, 1, "贿选应消耗外交点");
assert.ok(
