...
const partitionWorld = w.HIFI_WORLD_ENGINE.createWorld([
  tile(40, "法兰西王国", "巴黎", 0, "天主教", 16),
  tile(41, "布列塔尼公国", "南特", 2, "天主教", 10),
  tile(42, "纳瓦拉王国", "潘普洛纳", 4, "天主教", 5),
], undefined, "法兰西王国");
w.HIFI_DIPLOMACY_ENGINE.initializeDiplomacy(partitionWorld);
w.HIFI_SUPRANATIONAL_ENGINE.initializeSupranational(partitionWorld);
const partitionUnion = w.HIFI_SUPRANATIONAL_ENGINE.createPersonalUnion(partitionWorld, "法兰西王国", "布列塔尼公国", "复合王冠");
w.HIFI_SUPRANATIONAL_ENGINE.createPersonalUnion(partitionWorld, "法兰西王国", "纳瓦拉王国", "复合王冠");
partitionUnion.cohesion = 35;
partitionWorld.turn = 32;
partitionWorld.countries["法兰西王国"].leader.historicalEndAtTurn = 32;
w.HIFI_SUPRANATIONAL_ENGINE.processSupranational(partitionWorld);
assert.equal(
  w.HIFI_SUPRANATIONAL_ENGINE.unionFor(partitionWorld, "布列塔尼公国"),
  undefined,
  "低向心力复合君主国在主邦继承时应分割出最强从邦"
);
assert.equal(
  w.HIFI_SUPRANATIONAL_ENGINE.unionFor(partitionWorld, "纳瓦拉王国").head,
  "法兰西王国",
  "分割继承后剩余从邦仍应留在原共主结构"
);
assert.equal(partitionWorld.countries["布列塔尼公国"].lastUnionPartition.formerSenior, "法兰西王国", "分割继承应记录原主邦");
