...
france.government = politics.createGovernment("monarchy");
france.estates = politics.createEstates("monarchy");
world.tiles.filter(tile => tile.polity === "法兰西王国").forEach(tile => {
  tile.confession = "catholic";
  tile.religion = "天主教";
});
politics.enactDecision(world, "法兰西王国", "convert_reformed");
assert.equal(france.stateConfession, "lutheran", "接纳宗教改革应改变国家国教");
assert.equal(world.tiles.find(tile => tile.polity === "法兰西王国").confession, "lutheran", "接纳宗教改革应同步地块 confession");
