...
assert.ok(rules.buildings.shipyard);
assert.ok(rules.buildings.navalBase);
...
const portOnly = economy.tileOutput({ ...tiles[0], good: "naval_supplies", terrain: "coast", climate: "temperate", buildings: ["port"] }, country);
const navalBaseOutput = economy.tileOutput({ ...tiles[0], good: "naval_supplies", terrain: "coast", climate: "temperate", buildings: ["port", "shipyard", "navalBase"] }, country);
assert.ok(navalBaseOutput.military > portOnly.military, "船坞和军港必须提高海军军需产出");
...
assert.equal(economy.canConstructBuilding(world, "法兰西王国", 2, "shipyard").reason, "该建筑不适合当前地形", "内陆地块不能建设船坞");

const shipyardWorld = worldEngine.createWorld([
  { id: 90, isSea: false, polity: "港口测试国", population: 10, control: 90, good: "naval_supplies", terrain: "coast", climate: "temperate", buildings: ["port"], city: "港城", devastation: 0 },
], {}, "港口测试国");
economy.initializeEconomy(shipyardWorld);
const shipyardCountry = shipyardWorld.countries["港口测试国"];
shipyardCountry.money = 120;
shipyardCountry.actionPoints.administrative = 3;
assert.equal(economy.canConstructBuilding(shipyardWorld, "港口测试国", 90, "shipyard").ok, true, "沿海港口应允许建设船坞");
economy.constructBuilding(shipyardWorld, "港口测试国", 90, "shipyard");
assert.equal(economy.canConstructBuilding(shipyardWorld, "港口测试国", 90, "navalBase").ok, true, "船坞应解锁军港建设");
economy.constructBuilding(shipyardWorld, "港口测试国", 90, "navalBase");
assert.deepEqual(shipyardWorld.tiles[0].buildings, ["port", "shipyard", "navalBase"], "港口升级链必须按港口到船坞到军港落地");
...
vm.runInNewContext(fs.readFileSync(path.join(root, "data/faiths.js"), "utf8"), context);
vm.runInNewContext(fs.readFileSync(path.join(root, "data/trade.js"), "utf8"), context);
vm.runInNewContext(fs.readFileSync(path.join(root, "engine/faith.js"), "utf8"), context);
...
context.window.HIFI_FAITH_ENGINE.initializeFaith(world);
...
const straitWorld = worldEngine.createWorld([
  { id: 31, isSea: false, polity: "拜占庭帝国", population: 8, control: 80, good: "spices", buildings: ["port", "market"], city: "君士坦丁堡", devastation: 0 },
  { id: 32, isSea: false, polity: "马穆鲁克苏丹国", population: 7, control: 80, good: "spices", buildings: ["port", "market"], city: "亚历山大", devastation: 0 },
  { id: 33, isSea: false, polity: "威尼斯共和国", population: 6, control: 80, good: "cloth", buildings: ["port", "market"], city: "威尼斯", devastation: 0 },
  { id: 34, isSea: false, polity: "热那亚共和国", population: 6, control: 80, good: "cloth", buildings: ["port", "market"], city: "热那亚", devastation: 0 },
]);
context.window.HIFI_FAITH_ENGINE.initializeFaith(straitWorld);
trade.initializeTrade(straitWorld);
const levantBaseCost = trade.routeCost(straitWorld, straitWorld.trade.routes.levant);
assert.ok(levantBaseCost >= 9, "经过博斯普鲁斯的商路必须计入海峡通行成本");
trade.processTrade(straitWorld);
assert.ok(straitWorld.trade.lastStraitTolls["拜占庭帝国"]["博斯普鲁斯海峡"] > 0, "控制君士坦丁堡必须收取博斯普鲁斯海峡通行费");
straitWorld.tiles.find(tile => tile.city === "君士坦丁堡").polity = "威尼斯共和国";
trade.processTrade(straitWorld);
assert.ok(straitWorld.trade.lastStraitTolls["威尼斯共和国"]["博斯普鲁斯海峡"] > 0, "海峡通行费必须跟随控制者变化");

const tradeOnlyContext = { window: {} };
vm.runInNewContext(fs.readFileSync(path.join(root, "data/trade.js"), "utf8"), tradeOnlyContext);
vm.runInNewContext(fs.readFileSync(path.join(root, "engine/trade.js"), "utf8"), tradeOnlyContext);
assert.throws(
  () => tradeOnlyContext.window.HIFI_TRADE_ENGINE.initializeTrade({ countries: {}, tiles: [] }),
  /信仰系统/,
  "贸易系统不能回退到旧天主教压力算法，必须先加载信仰系统"
);
