...
assert.ok(mapSource.includes("waterTypeFor"), "地图必须标记可探索大洋海域");
assert.ok(mapSource.includes("isSeaDiscovered"), "地图迷雾必须读取海域发现状态");
assert.ok(componentSource.includes(".map-hex.fog"), "未知大洋必须有迷雾样式");
assert.ok(
