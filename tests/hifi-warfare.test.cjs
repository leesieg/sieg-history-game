...
const forcedTributary = diplomacy.subjectBetween(subjectWarWorld, "法兰西王国", "英格兰王国");
assert.ok(forcedTributary, "强制朝贡和约必须建立从属关系");
assert.equal(forcedTributary.terms.war, "自由宣战", "强制朝贡和约必须使用朝贡国高自治条款");

const subjectPermissionWorld = worldEngine.createWorld([
  { id: 30, isSea: false, polity: "宗主国", population: 12, buildings: [], city: "宗主城", terrain: "plains", x: 0, y: 0, control: 80, devastation: 0 },
  { id: 31, isSea: false, polity: "附属国", population: 6, buildings: [], city: "附属城", terrain: "plains", x: 20, y: 0, control: 70, devastation: 0 },
  { id: 32, isSea: false, polity: "第三国", population: 6, buildings: [], city: "第三城", terrain: "plains", x: 40, y: 0, control: 70, devastation: 0 },
]);
diplomacy.initializeDiplomacy(subjectPermissionWorld);
warfare.initializeWarfare(subjectPermissionWorld);
subjectPermissionWorld.diplomacy.wars = [];
subjectPermissionWorld.diplomacy.subjects.push({
  id: "subject-tributary", type: "tributary", overlord: "宗主国", subject: "附属国",
  autonomy: 78, loyalty: 62, tribute: 10, terms: diplomacy.subjectTerms(78), startedTurn: subjectPermissionWorld.turn,
});
assert.equal(warfare.canDeclareWar(subjectPermissionWorld, "附属国", "第三国").ok, true, "朝贡国应可自由对第三国宣战");
assert.equal(warfare.canDeclareWar(subjectPermissionWorld, "附属国", "宗主国").ok, false, "附属国攻击宗主不能走普通宣战");
subjectPermissionWorld.diplomacy.subjects[0] = {
  id: "subject-vassal", type: "vassal", overlord: "宗主国", subject: "附属国",
  autonomy: 48, loyalty: 55, tribute: 10, terms: diplomacy.subjectTerms(48), startedTurn: subjectPermissionWorld.turn,
};
assert.match(warfare.canDeclareWar(subjectPermissionWorld, "附属国", "第三国").reason, /宗主批准/, "附庸主动宣战必须被宗主权限制");
subjectPermissionWorld.diplomacy.subjects[0] = {
  id: "subject-puppet", type: "puppet", overlord: "宗主国", subject: "附属国",
  autonomy: 18, loyalty: 38, tribute: 20, terms: diplomacy.subjectTerms(18), startedTurn: subjectPermissionWorld.turn,
};
assert.match(warfare.canDeclareWar(subjectPermissionWorld, "附属国", "第三国").reason, /傀儡国/, "傀儡国不能主动宣战");
