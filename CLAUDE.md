...
整个世界是一台**每季自转的机器**，所有机制都围绕同一个闭环设计：

> **基底（地块·人口·资源）→ 各种流（产出 / 贸易 / 人口 / 科技 / 信仰）→ 压力层（`pressures` + 合法性 + 阶层满意）→ 转折层（纪元 / 情势 / 事件）→ 反作用回基底 → 循环**

- 机制（政治 / 经济 / 外交 / 军事 / 发展）不是循环里的「阶段」，而是挂在流上的**双向调节阀**：既改某条流的大小 / 方向 / 效率，又被流回灌（如阶层满意度被产出流分配喂养，不满了再惩罚流）。
- `HIFI_TURN_ENGINE.advanceQuarter` 每季按固定 8 步自动跑完这台机器（战争 → 补行动点 → 经济 → 贸易 → 外交 → 政治领导 → 历史 → AI），玩家不点任何键世界照样推进。
- **行动点**（行政 / 外交 / 军事，由统治者三维 `leaderActionGain` 决定）是玩家每季的「干预额度」：只 gate 自由动作（改法律 / 推改革 / 建建筑 / 整合 / 动员 / 外交提案），自动流不受其限制。**玩家是扰动这台机器，不是驱动它。**

设计任何机制前先回答三件事：**受什么流影响 / 影响什么流 / 现状如何展示**。空转的机制（拧了不影响任何流）要优先接进闭环。详见 `docs/design/19`（国家）、`docs/design/20`（经济 / 军事 / 外交 / 发展）。
...
`docs/design/` 为编号方案（01–20）：设计先行、实现说明随后——09 号=完整策划方案，10 号=Demo2 游戏层实现，11 号=历史因果引擎完整设计，15 号=全机制 Review 与框架图，**16 号=高保真界面优化与交付方案（hifi 主线的设计依据）**，17 号=hifi 主线机制现状与待修问题，18 号=hifi 交互组件全盘点，**19 号=国家机制深化设计（流模型总纲，新增机制范式）**，**20 号=经济/军事/外交/发展机制深化设计**。改动较大机制前先读对应编号。`docs/dev-diaries/` 是开发日志，`docs/research/` 是《欧陆风云 5》调研，`docs/plans/` 是历史实施计划。
